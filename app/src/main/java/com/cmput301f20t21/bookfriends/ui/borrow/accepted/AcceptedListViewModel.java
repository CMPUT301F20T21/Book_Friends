...
public class AcceptedListViewModel extends ViewModel {
    ...
    // production
    public AcceptedListViewModel() {
        this(AuthRepositoryImpl.getInstance(), BookRepositoryImpl.getInstance(), RequestRepositoryImpl.getInstance());
    }

    // test
    public AcceptedListViewModel(AuthRepository authRepository, BookRepository bookRepository, RequestRepository requestRepository){
    ...
    private void fetchBooks() {
        ...
        requestRepository.getAllRequestsByUsername(username, REQUEST_STATUS.ACCEPTED).addOnSuccessListener(requests -> {
            List<String> requestedBookIds = requests
                    .stream()
                    .map(Request::getBookId)
                    .collect(Collectors.toList());
            if (requestedBookIds.size() > 0) {
                bookRepository.batchGetBooks(requestedBookIds).addOnSuccessListener(acceptedBooks -> {
                    if (bookData != null) {
                        bookData.clear();
                        bookData.addAll(acceptedBooks);
